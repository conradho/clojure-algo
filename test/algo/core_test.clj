...
(deftest merge-and-count-test
  (testing "base cases"
    (is (= (merge-and-count nil [1] 5) [[1] 5]))
    (is (= (merge-and-count [2] nil 3) [[2] 3]))
    (is (= (merge-and-count [2 3] [1] 1) [[1 2 3] 3])))
  (testing "merging two sorted lists returns sorted output"
    (is (<= (first (merge-and-count (range 5) (range 5) 0))))
    (is (<= (first (merge-and-count (range 5) (range 5 10) 0))))
    (is (<= (first (merge-and-count (range 5 15) (range 5 10) 0)))))
  (testing "inversions counted correctly"
    (is (= (second (merge-and-count (range 5 10) (range 5) 0)) 25))
    ;; below is not 35, because it expects the input lists to be sorted
    (is (= (second (merge-and-count (range 10 5 -1) (range 5) 0)) 25))))
